...
export default function ProcessSection() {
  ...
  return (
    <section id="proceso" className="py-24 bg-white text-slate-950 relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="mb-20">
          ...
          <h2 className="text-4xl md:text-5xl font-headline font-semibold tracking-tighter text-slate-950">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-12">
          {steps.map((step, i) => (
            <div key={step.title} className="relative group">
              <div className="absolute -top-6 -left-6 text-6xl font-bold text-slate-100 group-hover:text-primary/10 transition-colors z-0">
                0{i + 1}
              </div>
              <div className="relative z-10 space-y-4">
                <div className="h-1 w-12 bg-primary mb-6" />
                <h3 className="text-xl font-headline font-bold tracking-tight text-slate-900">{step.title}</h3>
                <p className="text-slate-600 leading-relaxed">
                  {step.desc}
                </p>
                <div className="pt-4 border-t border-dotted border-slate-200 mt-4">
                  <span className="text-[10px] font-mono uppercase text-primary font-bold block mb-1">Stack/Method:</span>
                  <span className="text-xs font-mono text-slate-500">{step.technical}</span>
                </div>
              </div>

              {/* Connector lines for visual flow */}
              {i < steps.length - 1 && (
                <div className="hidden lg:block absolute top-1/2 -right-6 w-12 h-[1px] bg-slate-200 border-t border-dotted z-0" />
              )}
            </div>
          ))}
        </div>

        <div className="mt-24 p-8 border border-slate-100 bg-slate-50 flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="flex items-center gap-4 text-slate-950">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Todos nuestros sistemas incluyen documentación técnica completa y test coverage.</p>
            </div>
          </div>
